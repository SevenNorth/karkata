...
Karkata 0.1.0 的四个 workspace 已完成 Runtime、UI、双语文档站和发布物验证，但 npm registry 尚无公开包、仓库也没有版本 tag。需要在受控凭据和 GitHub 权限下完成首次公开发布，让使用方可以从 npm 安装稳定版本。

## 目标

- 将 `@karkata/core`、`@karkata/openai-compatible`、`@karkata/javascript` 和 `@karkata/ui` 以 `0.1.0` 发布到 npm public registry。
...
- npm scope 权限、2FA/OTP、trusted publishing 或网络状态可能阻塞首次发布。
...
- 发布者需在实施前确认 npm 登录账户拥有 `@karkata` scope 的 public publish 权限。
