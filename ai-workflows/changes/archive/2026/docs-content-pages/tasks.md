...
- [x] 8. 执行完整本地门禁和 production preview QA；有权限的发布者启用 Pages 并部署到默认项目地址。
- [x] 9. 完成部署后中英文、深层路由、搜索、资源、Demo、控制台和网络 smoke，验证成功后更新根/包 README 链接。
...
| 部署后 smoke | 初次 run `31992881568` 在 `configure-pages` 因 Pages 未启用失败 | 启用 Pages 后重新运行：build/deploy 成功，`a04d56b` artifact 发布到稳定 URL | 远端 26 路由、2 张截图、搜索、Demo、控制台和同源网络检查通过 |
...
| production/deployed browser QA | 通过 | 本地与远端均覆盖 26 路由、搜索、桌面/移动、Demo、控制台和同源网络 |
...
- `a04d56b` 已推送，GitHub Actions run `31992881568` 第二次运行部署成功；稳定 URL 已完成远端 smoke。
