...
`createUnsafeJavaScriptTool()` from `@karkata-ai/javascript` runs only explicitly registered, fully trusted code in the host's current Realm. It is not a sandbox and Core never enables it automatically.
