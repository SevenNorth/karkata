...
The main `@karkata/ui` entry does not access the DOM during import; only `/web-component` is browser-specific. With `panel.agent`, the component owns and disposes its internal Store when unbound. An external `panel.store` remains application-owned and can preserve the UI transcript across mounts.
